...
| --- | --- | --- |
| `POST` | `/links_claim.php` | привязать DroneBet-аккаунт к Sirius UID |
| `GET` | `/balance.php?sirius_uid={sirius_uid}` | получить баланс печенек |
| `POST` | `/credit.php` | начислить печеньки после списания коинов |
| `POST` | `/debit.php` | списать печеньки перед начислением коинов |
...
Курс фиксирован: **1 000 печенек DroneBet = 1 Сириус Коин**. В обмене
используются только целые коины, поэтому поле `amount` у DroneBet всегда
кратно `1000`.

### Контракт DroneBet

`POST /links_claim.php`:
...
коины, затем вызывает DroneBet `/credit.php`. Если DroneBet явно отклонил
...
`/debit.php`, затем начисляет коины у себя. Это исключает выпуск коинов без
